...
import { Award, FileCheck, History, Landmark, ShieldCheck, Download, Calendar, ArrowRight } from 'lucide-react';
...
export default function Akreditasi({ institusi, prodis, riwayats, pengaturan }) {
    ...
    const akreditasiInstitusi = {
        ...
        sertifikat_url: institusi?.sertifikat_url || '#',
        sk_url: institusi?.sk_url || '#',
        ...
        <PublicLayout>
                    ...
                    <div className="bg-white rounded-lg shadow-lg border border-slate-200 overflow-hidden mb-16 animate-fade-in-up" style={{animationDelay: '0.1s'}}>
                        <div className="grid grid-cols-1 md:grid-cols-3">
                            <div className="bg-emerald-800 p-8 flex flex-col items-center justify-center text-center text-white">
                                ...
                                <div className="text-4xl font-extrabold text-white tracking-tight mb-2">{akreditasiInstitusi.peringkat}</div>
                            ...
                            <div className="md:col-span-2 p-8 md:p-10 flex flex-col justify-center">
                                ...
                                <div className="mt-8 pt-6 border-t border-slate-100 flex flex-wrap gap-4">
                                    <a href={akreditasiInstitusi.sertifikat_url} className="inline-flex items-center gap-2 px-5 py-2.5 bg-emerald-600 text-white rounded-lg font-semibold hover:bg-emerald-700 transition-colors shadow-md shadow-emerald-200">
                                        <Download className="w-4 h-4" /> Unduh Sertifikat
                                    </a>
                                    <a href={akreditasiInstitusi.sk_url} className="inline-flex items-center gap-2 px-5 py-2.5 bg-white text-emerald-700 border border-emerald-200 rounded-lg font-semibold hover:bg-emerald-50 transition-colors">
                                        Unduh SK Akreditasi
                                    </a>
                                </div>
                            </div>
                        ...
                        <div className="lg:col-span-2 space-y-6 animate-fade-in-up" style={{animationDelay: '0.2s'}}>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                {akreditasiProdi.length > 0 ? akreditasiProdi.map((prodi, index) => (
                                    <div key={index} className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 hover:shadow-md transition-shadow relative overflow-hidden group">
                                        ...
                                        <h4 className="font-bold text-slate-800 text-lg mb-4">{prodi.nama}</h4>
                            ...
                            <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                                ...
                                <div className="space-y-6">
                                    {riwayatAkreditasi.length > 0 ? riwayatAkreditasi.map((item, index) => (
                                        <div key={index} className="relative pl-6 before:absolute before:left-0 before:top-2 before:w-2 before:h-2 before:bg-emerald-500 before:rounded-full after:absolute after:left-1 after:top-4 after:w-0.5 after:h-full after:bg-emerald-100 last:after:hidden">
                                            <p className="text-sm font-bold text-slate-800">{item.nama}</p>
                                            <p className="text-emerald-600 font-semibold my-0.5">Peringkat: {item.peringkat}</p>
            ...
            {showWaPopup && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm transition-all duration-300">
                    ...
                    ></div>
                    <div className="bg-white rounded-xl shadow-2xl w-full max-w-sm overflow-hidden animate-fade-in-up relative z-10 border border-slate-200">
                        <div className="bg-emerald-600 p-4 text-white flex items-center gap-3">
                            <div className="w-10 h-10 bg-white/20 rounded-full flex items-center justify-center">
                                <ShieldCheck className="w-6 h-6" />
                            </div>
                            <div>
                                <h4 className="font-bold">Admin LPM</h4>
                        ...
                        <div className="p-6">
                            ...
                            <a 
                                href={`https://wa.me/${akreditasiInstitusi.whatsapp_lpm?.replace(/^0/, '62')}?text=Halo%20Admin%20LPM%20Pascasarjana%20IAI%20Persis%20Bandung`} 
                                target="_blank"
                                rel="noreferrer"
                                className="w-full inline-flex items-center justify-center gap-2 px-5 py-3 bg-emerald-500 text-white rounded-lg font-bold hover:bg-emerald-600 transition-colors shadow-md"
                            >
